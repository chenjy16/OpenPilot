...
describe('Sidebar', () => {
  ...
  it('renders session list placeholder when no sessionList prop', () => {
    render(<Sidebar />);
    expect(screen.getByText('暂无会话，点击上方按钮新建一个吧')).toBeDefined();
  });
  ...
  it('has a scrollable session list area', () => {
    render(<Sidebar />);
    const scrollArea = screen.getByText('暂无会话，点击上方按钮新建一个吧').closest('div');
  ...
  it('has a bottom border on the model selector area', () => {
    render(<Sidebar />);
    const modelArea = screen.getByText('模型').closest('div');
