...
const DebugView: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      <h2 className="text-xl font-semibold text-gray-800">{t('debug.title')}</h2>

      {/* Snapshots grid */}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <SnapshotCard title={t('debug.systemStatus')} content={statusJson} />
        <SnapshotCard title={t('debug.healthCheck')} content={healthJson} />
        <SnapshotCard title={t('debug.modelDirectory')} content={modelsJson} />
        <SnapshotCard title={t('debug.heartbeat')} content={heartbeatJson || t('debug.noData')} />
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4">
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-sm font-medium text-gray-700">
            {t('debug.eventLog', { count: events.length, limit: EVENT_LOG_LIMIT })}
          </h3>
        ...
        <div ref={eventsRef} className="max-h-64 overflow-auto rounded bg-gray-50 p-2 font-mono text-xs">
          {events.length === 0 && <div className="text-gray-400">{t('debug.waitingForEvents')}</div>}
          {events.map((e) => (
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4">
      <div className="mb-2 flex cursor-pointer items-center justify-between" onClick={() => setCollapsed(!collapsed)}>
        ...
        <span className="text-xs text-gray-400">{collapsed ? t('debug.expand') : t('debug.collapse')}</span>
      </div>
      {!collapsed && (
        <pre className="max-h-48 overflow-auto rounded bg-gray-50 p-3 font-mono text-xs text-gray-700">
          {content || t('debug.loading')}
        </pre>
      )}
    </div>
  );
