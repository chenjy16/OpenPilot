...
const SECTION_META: Record<string, { icon: string; label: string; description: string; order: number }> = {
  ...
  ui: { icon: '🎨', label: 'UI 外观', description: 'Web UI 主题色、助手名称', order: 24 },
  cli: { icon: '💻', label: 'CLI 配置', description: 'CLI 横幅与标语模式', order: 25 },
  secrets: { icon: '🔒', label: '密钥管理', description: '密钥来源提供商', order: 26 },
  env: { icon: '🌱', label: '环境变量', description: '环境变量注入与 Shell 导入', order: 27 },
  meta: { icon: '📝', label: '配置元数据', description: '配置文件版本与时间戳', order: 28 },
  apiKeys: { icon: '🔑', label: 'API 密钥', description: '环境变量设置', order: 29 },
  nodeEnv: { icon: '⚙️', label: '运行环境', description: '环境标识', order: 30 },
  logLevel: { icon: '📊', label: '全局日志级别', description: '日志级别', order: 31 },
  databasePath: { icon: '💾', label: '数据库路径', description: 'SQLite 路径', order: 32 },
  debug: { icon: '🐛', label: '调试模式', description: '调试输出', order: 33 },
};
